...
namespace crg::ecs {
    ...
    class QueryResult {
    public:
        QueryResult(std::vector<std::vector<Chunk>*> chunkLists) :
        ...
        Iterator begin() {
            return Iterator(&m_chunkLists, 0, 0, 0);
        }

        Iterator end() {
            return Iterator(&m_chunkLists, m_chunkLists.size(), 0, 0);
        }

    private:

        std::vector<std::vector<Chunk>*> m_chunkLists;
